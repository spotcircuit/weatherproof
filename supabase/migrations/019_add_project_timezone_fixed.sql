...
-- Create a view for weather with local times (no generated columns)
...
-- Show current project timezones after update
SELECT 
  name,
  address,
  timezone,
  CURRENT_TIMESTAMP AT TIME ZONE timezone as local_time
FROM projects
WHERE active = true
ORDER BY name;